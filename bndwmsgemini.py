...
class BndWmsApplication:
    def __init__(self, page: ft.Page) -> None:
        ...
        # tabs_root инициализируем позже, когда узнаем пользователя
        self.tabs_root: ft.Tabs | None = None
        self.settings_users_column: ft.Column | None = None

    def _build_tabs(self) -> ft.Tabs:
        # Согласно Flet 0.84.0, используем TabBar и TabBarView внутри Tabs
        tab_bar = ft.TabBar(
            tabs=[ft.Tab(label=title) for title in self._tab_titles], # 'text' заменен на 'label'
            scrollable=False,
        ...
        tab_view = ft.TabBarView(
            ...
            controls=[
                ...
                self._settings_tab_body(), # Здесь теперь корректно сработает проверка на Admin
            ],
        )
    ...
    def _login_screen(self) -> ft.Control:
        ...
        def sign_in(_: ft.ControlEvent) -> None:
            ...
            self.page.controls.clear()
            # Пересоздаем основной экран с учетом авторизованного пользователя
        ...
        return ft.Container(
            ...
            content=ft.Container(
                ...
                border_radius=16,
                border=ft.Border.all(1, COLORS["border"]), # 'border.all' заменен на 'Border.all'
                content=ft.Column(
                    tight=True,
                    spacing=12,
                    controls=[
                        ft.Text("Вход в BnDWMS", size=22, weight=ft.FontWeight.W_600),
                        ft.Text("Для регистрации обратитесь к администратору", size=12, color=COLORS["muted"]),
                        fio,
                        pin,
                        warning,
                        ft.FilledButton(
    ...
    def _main_screen(self) -> ft.Control:
        # Инициализируем вкладки только здесь, когда известен пользователь
    ...
    def _settings_tab_body(self) -> ft.Control:
        ...
        # Теперь проверка на Admin сработает, так как метод вызывается после авторизации
    ...
    def _on_hotkey(self, event: ft.KeyboardEvent) -> None:
        ...
        # В Flet 0.84.0 Tabs синхронизирует TabBar и TabBarView через selected_index
